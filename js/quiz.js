...
async function syncCloud() {
  try { const user = await getCurrentUser(); if (user) await pushToCloud(); } catch(e) {}
}
...
function removeFromReview(wordId) {
  try {
    const w = JSON.parse(localStorage.getItem('fg_wrong') || '{}');
    if (w[wordId] > 1) w[wordId]--; else delete w[wordId];
    localStorage.setItem('fg_wrong', JSON.stringify(w));
  } catch(e) {}
}
...
function checkAnswer(skipped = false) {
  ...
  if (skipped) {
    ...
    if (addBtn) addBtn.style.display = 'none';
    // 答错朗读正确答案
    setTimeout(() => speak(q.word, 0.8), 500);
  }
...
function showResults() {
  ...
  const scenePcts = {};
  SCENES.forEach(s => {
    const sc = sceneScores[s.key];
    scenePcts[s.key] = sc.total > 0 ? Math.round((sc.correct / sc.total) * 100) : 30;
  });
  ...
  setTimeout(() => {
    drawRadar(scenePcts);
    renderDimBars(scenePcts);
    if (window.renderWeakReco) window.renderWeakReco(scenePcts, SCENES);
  }, 200);
}
...
function drawRadar(scenePcts) {
  ...
  // 修复：有背景图才是深色，white/black 都用 dm 浅色方案
  const isDark     = !!document.body.style.backgroundImage || (!document.body.classList.contains('dm'));
  ...
  angles.forEach((a, i) => {
    ...
    ctx.fillStyle = SCENES[i].color; ctx.fill();
  });
}
  ...
  SCENES.forEach(s => {
    ...
    row.innerHTML = `
      ...
      <div class="dim-bar-track">
        <div class="dim-bar-fill" data-pct="${pct}" style="background:${s.color}"></div>
      </div>
...
btnStartLearn.addEventListener('click', () => { navigate('learn.html'); });
