...
export const DIMENSIONS = [
  ...
  { key:'emotion',  label:'情感心理', color:'rgba(255,170,210,.85)', icon:'💭' },
  { key:'nature',   label:'自然世界', color:'rgba(140,240,180,.85)', icon:'🌿' },
  { key:'abstract', label:'抽象概念', color:'rgba(190,160,255,.85)', icon:'✦'  },
  { key:'social',   label:'社交场景', color:'rgba(255,175,100,.85)', icon:'🗣'  },
  { key:'academic', label:'学术书面', color:'rgba(100,230,210,.85)', icon:'📖' },
];
...
const QUESTION_BANK = [
  ...
  {dim:'daily',level:2,sentence:'We need to replace the kitchen ___ — the fridge is broken.',zh:'我们需要更换厨房家电。',answer:'appliance',hint:'家电（单个）'},
  ...
  {dim:'daily',level:6,sentence:'She found poetry in the ___ details of everyday life.',zh:'她在生活的日常细节中发现了诗意。',answer:'quotidian',hint:'书面词，日常平凡的'},
  ...
  {dim:'emotion',level:4,sentence:'I feel ___ about leaving — excited but also scared.',zh:'对于离开，我心情很矛盾。',answer:'ambivalent',hint:'矛盾的，态度两可'},
  ...
  {dim:'nature',level:6,sentence:'After the rain, the ___ of wet earth filled the air.',zh:'雨后，空气中弥漫着湿润泥土的气息。',answer:'petrichor',hint:'雨后泥土的气息'},
...
function checkAnswer(skipped = false) {
  ...
  if (ok) {
    score++;
    if (dimScores[q.dim]) dimScores[q.dim].correct++;
  }
...
function showResults() {
  ...
  setTimeout(() => {
    drawRadar(dimPcts);
    renderDimBars(dimPcts);
    // 薄弱维度推荐
    if (window.renderWeakReco) window.renderWeakReco(dimPcts, DIMENSIONS);
  }, 200);
}
...
function drawRadar(dimPcts) {
  ...
  DIMENSIONS.forEach((d,i)=>{
    ...
    ctx.fillStyle=labelColor; ctx.globalAlpha=.9;
  ...
  ctx.strokeStyle='rgba(200,230,255,.6)'; ctx.lineWidth=1.5; ctx.stroke();
  ...
  angles.forEach((a,i)=>{
    ...
    ctx.fillStyle=DIMENSIONS[i].color; ctx.fill();
  });
}
